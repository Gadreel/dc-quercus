		...
		switch($dest) {
			...
			case 'FD': {
				...
				if ($this->diskcache) {
					...
					fwrite2($f, $this->getBuffer(), $this->bufferlen);
					fclose($f);
				}
...
Putting 1 is equivalent to putting 0 and calling Ln() just after. Default value: 0.
	...
	protected function writeDiskCache($filename, $data, $append=false) {
		...
		$f = @fopen($filename, $fmode);
    		...
    		dc_debug("write2: " . $data);
    		
		if (!$f) {
    		dc_debug("write3: " . strlen($data));
    		
			$this->Error('Unable to write cache file: '.$filename);
		} else {
    		dc_debug("write4: " . strlen($data));
    		
			fwrite2($f, $data);
			fclose($f);

    		dc_debug("write5: " . strlen($data));
		}
